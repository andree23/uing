...
class MovieController extends Controller
{
    ...
    public function show($movie)
    {
        ...
        $movie = Movie::where('tmdb_id', '=', $movie)->first();
        ...
        if ($request->linksubs) {
            foreach ($request->linksubs as $substitle) {
                if (!isset($substitle['id'])) {
                    if (!filter_var($substitle['link'], FILTER_VALIDATE_URL)) {
                        $embed = new Embed();
                        $embed->code = $substitle['link'];
                        $embed->save();
                        $substitle['link'] = $request->root() . '/api/embeds/show/' . $embed->id;
                    }
    ...
    public function videoDestroy(MovieVideo $video)
    {
        if ($video != null) {
            ...
            $video->delete();
            $file = public_path() . '/substitles/' . $video;
            @unlink($file);
    ...
    public function recommended($statusapi)
    {
        ...
        return response()->json(['recommended' => $movies], 200);
    }
    ...
    public function trending($statusapi)
    {
        ...
        $movies = Movie::orderByDesc('views')->limit(10)
            ->get();
...

    {
        ...
        $movies = Movie::where('featured', 1)->where('created_at', '>', Carbon::now()
            ->subMonth())
            ->orderByDesc('created_at')
            ->get();

        $series = Serie::where('featured', 1)->where('created_at', '>', Carbon::now()
            ->subMonth())
            ->orderByDesc('created_at')
            ->get();
...

    {
        ...
        $series = Serie::inRandomOrder()->limit(4)->get();
    ...
    public function popular($statusapi)
    {
        ...
        $movies = Movie::orderByDesc('popularity')->limit(5)
            ->get();
    ...
    public function kids($statusapi)
    {
